...
$INTERPRETER $SCRIPT --input=${LOCAL_IN_PATH}test01.in --output=${LOCAL_OUT_PATH}test01.out --determinization 2> ${LOG_PATH}test01.err
echo -n $? > ${LOG_PATH}/test01.!!!

# test02: jednoducha determinizace (z prednasek); Expected output: test02.out; Expected return code: 0
$INTERPRETER $SCRIPT --input=${LOCAL_IN_PATH}test02.in -d > ${LOCAL_OUT_PATH}test02.out 2> ${LOG_PATH}test02.err
echo -n $? > ${LOG_PATH}/test02.!!!

# test03: pokrocila determinizace; Expected output: test03.out; Expected return code: 0
$INTERPRETER $SCRIPT -d --input=${LOCAL_IN_PATH}test03.in --output=${LOCAL_OUT_PATH}test03.out 2> ${LOG_PATH}test03.err
echo -n $? > ${LOG_PATH}/test03.!!!

# test04: diakritika a UTF-8 na vstupu; Expected output: test04.out; Expected return code: 0
$INTERPRETER $SCRIPT < ${LOCAL_IN_PATH3}test04.in > ${LOCAL_OUT_PATH}test04.out 2> ${LOG_PATH}test04.err
...
$INTERPRETER $SCRIPT --input=${LOCAL_IN_PATH}test05.in --output=${LOCAL_OUT_PATH}test05.out 2> ${LOG_PATH}test05.err
echo -n $? > ${LOG_PATH}/test05.!!!

# test06: vstup ze zadani; Expected output: test06.out; Expected return code: 0
$INTERPRETER $SCRIPT -d --output=${LOCAL_OUT_PATH3}test06.out < ${LOCAL_IN_PATH}test06.in 2> ${LOG_PATH}test06.err
...
$INTERPRETER $SCRIPT -e --input=${LOCAL_IN_PATH}test07.in --output=${LOCAL_OUT_PATH2}test07.out 2> ${LOG_PATH}test07.err
echo -n $? > ${LOG_PATH}/test07.!!!

# test08: Chybna kombinace parametru -e a -d; Expected output: test08.out; Expected return code: 1
$INTERPRETER $SCRIPT --input=${LOCAL_IN_PATH}test08.in --output=${LOCAL_OUT_PATH3}test08.out -d --no-epsilon-rules 2> ${LOG_PATH}test08.err
echo -n $? > ${LOG_PATH}/test08.!!!

# test09: chyba vstupniho formatu; Expected output: test09.out; Expected return code: 40
$INTERPRETER $SCRIPT --input=${LOCAL_IN_PATH2}test09.in --output=${LOCAL_OUT_PATH}test09.out 2> ${LOG_PATH}test09.err
...
$INTERPRETER $SCRIPT --input=${LOCAL_IN_PATH3}test10.in --output=${LOCAL_OUT_PATH}test10.out 2> ${LOG_PATH}test10.err
...
$INTERPRETER $SCRIPT --input=${LOCAL_IN_PATH}test90.in --output=${LOCAL_OUT_PATH2}test90.out --rules-only 2> ${LOG_PATH}test90.err
...
$INTERPRETER $SCRIPT --input=${LOCAL_IN_PATH3}test91.in --output=${LOCAL_OUT_PATH}test91.out --white-char 2> ${LOG_PATH}test91.err
...
$INTERPRETER $SCRIPT -w --input=${LOCAL_IN_PATH}test92.in -r --output=${LOCAL_OUT_PATH3}test92.out 2> ${LOG_PATH}test92.err
